...
// template <>
// struct NumberToAbs<std::complex<double>> {
...
// template <typename T>
// __global__ void matrix_l1_norm_kernel(T *d_A, T *d_result, int M)
// {
...
//         {
//             sum+= NumberToAbs<T>::abs(d_A[col*M + row]); //我的矩阵是以列为主序的
...
//     T *d_result;
//     cudaMalloc((void **)&d_result,M*sizeof(T));
//     dim3 blockSize(256);
//     dim3 gridSize((M+blockSize.x - 1) / blockSize.x);

//     matrix_l1_norm_kernel<double><<<gridSize, blockSize>>>(d_A,d_result,M);
//     cudaDeviceSynchronize();
//     int L1_norm_idx;
//     double *L1_result = (double *)malloc(M*sizeof(double));;
//     cublasAPI<T>::I_amax(cublasH,M,d_result,1,&L1_norm_idx);
//     cudaMemcpy(L1_result,d_result,M*sizeof(T),cudaMemcpyDeviceToHost);
//     // 得到转化后的A的第一范数
//     cudaFree(d_result);
